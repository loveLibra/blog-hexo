...
<!-- more -->
### Expressions
表达式就是{% raw %}{{}}{% endraw %}里面包的东西，Handlebars中语法的最小单位。{% raw %}{{xx}}{% endraw %}表示在**当前上下文环境**中去寻找xx属性，并用xx属性的值去替换之。Handlebars支持`.`分隔的属性{% raw %}{{xx.yy}}{% endraw %}，但在有的情况下`.`不能解决我们的问题，比如:
```javascript
    data = {
        articles: [
            {
...
前面讲了`<b>`实现了加粗，那加粗总该有个对象吧，也就是{% raw %}My name is {{name}}{% endraw %}； 
...
* {% raw %}{{#each arg}}{% endraw %}
循环内可以通过{% raw %}{{@index}}{% endraw %}获取当前的循环的索引， 通过{% raw %}{{@../index}}{% endraw %}可以获得父级遍历的索引；  
{% raw %}{{@key}}{% endraw %}可以获取当前遍历的数组或者对象的键值，数组中等同于{% raw %}{{@index}}{% endraw %}；  
{% raw %}{{@first}}{% endraw %}，{% raw %}{{@last}}{% endraw %}表示遍历中的第一个和最后一个的标识，返回true或false；  
each可选择性插入{% raw %}{{else}}{% endraw %}， 在遍历的list为**空**(遍历对象为非空数组或者对象时不为空值)时执行。

* {% raw %}{{#if condition}} or {{else}} or {{else if conditionElse}}{% endraw %}

* {% raw %}{{#unless condition}} inverse of the `if` helper{% endraw %}
...
```
* {% raw %}{{lookup context key}}{% endraw %} 获取动态参数进行渲染，相当于获取context.key的值，看例子会容易理解点：
```html
...
* {% raw %}{{log info}}{% endraw %} 打印调试

### Partials
Handlebars.registerPartial('parName', 'parContent'); 注册一个Partials；

需要调用Partials时{% raw %}{{> parName}}{% endraw %}即可。

调用Partials时也可以传入参数：{% raw %}{{> parName parContext}}指定Partials的执行上下文；{{> parName attr=val}}{% endraw %}

### Path
{% raw %}{{xx}}{% endraw %}中的xx可以不只是我们熟悉的一个属性的名字，或是xx.yy这样的嵌套路径。也能支持`../`这样的父级上下文环境和`./`这样的当前上下文环境的语法啦，来个例子：
...
### Comments
{% raw %}{{!-- --}}和{{! }}{% endraw %}不会将注释内容输出到HTML中，如果想输出注释到HTML中可以使用HTML的注释语法`<!-- -->`。另外，如果注释中有{% raw %}}}{% endraw %}，则必须使用{% raw %}{{!-- --}}{% endraw %}。
